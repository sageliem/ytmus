...
void TuiApp::load_buffer(const int id, std::unique_ptr<Player> &p) {
...
void BufferWindow::setup(std::unique_ptr<Player> &p_init, const int h_init,
...
void BufferWindow::update() {
  ...
  if (p) {
    ...
    mvwprintw(win, 2, 2, "loop[%.2f, %.2f]", p->getLoopStart(),
              p->getLoopEnd());
...
void BufferWindow::close() { delwin(win); }
